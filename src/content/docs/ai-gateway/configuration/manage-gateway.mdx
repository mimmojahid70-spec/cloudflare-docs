...
The request that triggers auto-creation must be authenticated. Include a valid `cf-aig-authorization` header or use a [Workers binding](/ai-gateway/integrations/aig-workers-ai-binding/). An unauthenticated request to a `default` gateway that does not yet exist does not create the gateway.

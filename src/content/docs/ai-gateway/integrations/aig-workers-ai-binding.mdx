...
In the next step, you will use the [default gateway](/ai-gateway/configuration/manage-gateway/#default-gateway) by setting the gateway ID to `default`. AI Gateway creates this gateway automatically on the first request.
...
```typescript title="src/index.ts" {15-18}
